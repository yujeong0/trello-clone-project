...
export default {
    ...
    methods: {
        ...
        setCardDraggable() {
          ...
          this.cDragger = dragger.init(Array.from(this.$el.querySelectorAll('.card-list'))) //배열로 받은 결과를 분해하여 전달... 그런데 대괄호로 감싸져있으니 다시 배열됨)
          ...
          this.cDragger.on('drop', (el, wrapper, target, siblings) => {
            ...
            if (!prev && next)  // 맨 앞에 있다면
              targetCard.pos = next.pos / 2
            else if(!next && prev)  // 맨 뒤에 있다면
              targetCard.pos = prev.pos * 2
            else if(prev && next)  // 중간에 삽입되어있다면
        ...
        onClickTitle() {
          ...
          this.$nextTick(() => this.$refs.inputTitle.focus())  // 실행 지연..?
        },
        onSubmitTitle() {
